		...
		while (true){
			...
			for (Update update : updates) {
				...
				if(this.searchBehaviour==true){
					...
					setControllerSearch(new ControllerAdd(model, this));
					
//					String string<arrayList> String();
					
					sendResponse = bot.execute(new SendMessage(update.message().chat().id(),"Digite o nome do contato, e o numero"));
					this.searchBehaviour = true;
					
				}
				
				else {
					sendResponse = bot.execute(new SendMessage(update.message().chat().id(),"Ola, eu sou a trixy, sua assistente de contatos\n"
					+ "Entre com 1 para adicionar contato\n"
					+ "Entre com 2 para remover\n"
					+ "Entre com 3 para editar\n"
					+ "Entre com 4 para listar\n"
					+ "Entre com 5 para pesquisar um contato existente"));
				}
